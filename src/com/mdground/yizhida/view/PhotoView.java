...
public class PhotoView extends ImageView {
	...
	ImageClick imgeClick;

	public ImageClick getImgeClick() {
		return imgeClick;
	}

	public void setImgeClick(ImageClick imgeClick) {
		this.imgeClick = imgeClick;
	}
	 ...
	 */
	public PhotoView(Context context, AttributeSet paramAttributeSet) {
	 ...
	 * @param h
	 */
	public PhotoView(Context context, int w, int h) {
		super(context);
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP:
			...
			up_y = (int) event.getRawY();

			if (Math.abs(up_x - down_x) < 10 && Math.abs(up_y - down_y) < 10 && mode != ZOOM) {
				imgeClick.onImageClick();
			}
			int disX = 0;
			int disY = 0;
			...
			if (getWidth() <= screenW) {
										 * if (this.getLeft() < 0) { disX =
										 * getLeft(); this.layout(0,
										 * this.getTop(), 0 + getWidth(),
										 * this.getBottom()); } else if
										 * (this.getRight() > screenW) { disX =
										 * getWidth() - screenW + getLeft();
										 * this.layout(screenW - getWidth(),
										 * this.getTop(), screenW,
										 * this.getBottom()); }
