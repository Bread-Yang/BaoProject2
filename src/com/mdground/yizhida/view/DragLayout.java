...
public class DragLayout extends FrameLayout {
    ...
    private MainRelativeLayout vg_main;
    ...
    protected void onFinishInflate() {
        ...
        vg_main = (MainRelativeLayout) getChildAt(isShowShadow ? 2 : 1);
...
/*    @Override
    public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MASK:
			...
			*//***
			 * 解决与子控件viewpager左右滑动冲突问题
			 *//*
			 int count = PreferenceUtils.getPrefInt(this.getContext(), MemberConstant.SHOW_LIST_COUNT,0);
			 int showPage = PreferenceUtils.getPrefInt(this.getContext(), MemberConstant.SHOW_FRAGMENT_PAGE,R.id.wait_button);
			if(showPage == R.id.wait_button && count != 0){
				if(count < 0){
					PreferenceUtils.setPrefInt(this.getContext(),MemberConstant.SHOW_LIST_COUNT,0);
				}
				return false;
			}
			 if (!isOpen()) {
...
//				if ( Math.abs(downX - actionX) >= Math.abs(downY - actionY) && downX - actionX > 0) {
//					return false;
//				}
			}
		}
